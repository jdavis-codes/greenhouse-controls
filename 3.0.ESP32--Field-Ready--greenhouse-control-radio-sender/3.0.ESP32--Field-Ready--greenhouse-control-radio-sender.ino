/*The sketch below runs the FIELD NODE (Arduino Nano ESP32 + LoRa radio), a greenhouse control node.
INPUTS:
...
NOTE: The ESP32 ADC is 12-bit (0-4095). The LoRa radio uses hardware Serial1 on the RX/TX pins,
so D2 and D3 are free for DHT22 unlike on the classic Arduino Nano.
*/
...
// LoRa radio uses hardware Serial1 on the board's RX and TX pins
#define LORA_RX RX
#define LORA_TX TX
...
#define insolationPin    A6 //analog read for the light meter (12-bit ADC: 0-4095)
#define soilMoisturePin  A7 //analog read for the soil moisture probe (12-bit ADC: 0-4095)
...
void setup() {
  ...
  switch (data_pipe) {
    case radio: {
      ...
      Serial.println(F("Greenhouse sender ready on RADIO pipe."));
      break;
    }
    case uart_rx_tx: {
      Serial1.begin(115200, SERIAL_8N1, LORA_RX, LORA_TX);
      greenhouseNode.begin(nullptr, REMOTE_ADDRESS, data_pipe, &Serial1);
      Serial.println(F("Greenhouse sender ready on UART RX/TX pipe."));
      break;
    }
  }
}
...
void readSensors() {
  ...
  //now read the soil moisture and convert to 0-100 (ESP32 ADC is 12-bit: 0-4095)
  soilMoisture = map(analogRead(soilMoisturePin), 0, 4095, 100, 0);

  //now read the light level and convert to 0-100 (ESP32 ADC is 12-bit: 0-4095)
  insolation = map(analogRead(insolationPin), 0, 4095, 0, 100);
}
