...
## Flashing Multiple Arduino Nano ESP32 Boards on macOS

When working with multiple Arduino Nano ESP32 boards simultaneously, you may encounter an error like:
...
### The Cause
The Arduino Nano ESP32 exposes a native USB DFU interface. By default, PlatformIO's `dfu-util` command does not specify *which* device to flash, so having multiple boards plugged in confuses the flasher. 

Switching to `upload_protocol = esptool` is **not recommended**. Doing so resets the ESP32 via serial DTR/RTS lines, throwing it into the hardware ROM bootloader. This changes the USB identity, disconnects the active port, creates a generic port (like `/dev/cu.usbmodem101`), and predictably causes the initial flash to fail.

### The Solution
We keep the default `dfu` upload protocol and dynamically pass the correct device serial number to `dfu-util`. 

On macOS, the port name automatically includes the device's MAC address (e.g., `/dev/cu.usbmodemE4B063AE16A82`). Our PlatformIO pre-script (`set_src_dir.py`) intercepts the upload task, parses the MAC address using a regex, and prepends the `-S <MAC>` flag to the `dfu-util` command. This ensures the intended device is reliably flashed without disconnecting the port or switching to `esptool`.
