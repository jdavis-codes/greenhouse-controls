...
void setup() {
  ...
  initializePins(); //set up pins as input or output and write the pins high for turning on the temp/hum sensors
...
if (fanOn == false){ //if the fan is off...
  if (grnhouseTemp >= (grnhouseTargetTemp2 + grnhouseTempDelta)){ //and if the greenhouse is hotter than the target...
    ...
    digitalWrite (relayOnePin, HIGH); //this activates the 120 vac relay for the louver
...
if (fanOn == true){ //if the sides
  if (grnhouseTemp <= (grnhouseTargetTemp2 - grnhouseTempDelta)){ //this checks if the greenhouse is colder than the target, and...
    ...
    digitalWrite (relayOnePin, LOW); //this opens the relay and turns off the fan
    //close the louver
