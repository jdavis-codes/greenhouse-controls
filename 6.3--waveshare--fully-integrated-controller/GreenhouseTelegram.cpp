...
void GreenhouseTelegramBot::begin(const char* ssid, const char* pass, 
    ...
    // ESP32 Secure Clients require correct time to validate TLS certificates.
    // Sync time with NTP (Greenwich Mean Time)
    ...
    // Note: FastBot2 exposes `client` natively and runs `client.setInsecure()` in its constructor by default on ESP32 Core 3+ (and BearSSL), 
    // so `skipCert()` is already effectively active, but it still requires a valid GMT time offset context to initiate TLS.
    // However, if we're STILL getting EOF errors, let's explicitly invoke it here:
    bot.client.setInsecure();
...
String GreenhouseTelegramBot::formatTime(uint32_t ts) {
    if (ts == 0) return "--:--";
    
    // Quick and dirty manual time conversion from epoch
    // Seconds in a day is 86400
    uint32_t secondsInDay = ts % 86400;
